        ...
        if (currentRandomValue !== lastRandomValueRef.current) {
          ...
          if (systemId && values.length >= 21) {
            ...
            logInverterData(systemId, dataToLog)
              .then(success => {
                if (success) console.log("Successfully logged inverter data to Supabase");
              })
              .catch(err => {
                console.error("Failed to log inverter data:", err);
              });
          }
        }
      }
        ...
        if (currentRandomValue !== lastRandomValueRef.current) {
          ...
          if (systemId && data) {
            logInverterData(systemId, {
              ...
              .then(success => {
                if (success) console.log("Successfully logged Firebase data to Supabase");
              })
              .catch(err => {
                console.error("Failed to log Firebase data:", err);
              });
          }
        }
      }
