...
import { runLastSeenMigration } from "@/utils/databaseMigrations";
...
const Dashboard = () => {
  ...
  // Run database migrations on initial load
  useEffect(() => {
    const ensureDbSetup = async () => {
      try {
        console.log("Ensuring database is properly set up...");
        const result = await runLastSeenMigration();
        if (result.success) {
          console.log("Database migration successful:", result);
        } else {
          console.error("Database migration failed:", result.error);
          // Don't show an error toast, it would be confusing to users
        }
      } catch (err) {
